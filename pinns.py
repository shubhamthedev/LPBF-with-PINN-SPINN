...
# ─────────────────────────────────────────────────────────────────────────────
# 2.  Material Properties — Hastelloy X 
# ─────────────────────────────────────────────────────────────────────────────
def kappa(T):
    return (229.87 + 0.0184 * T
            + 225.10 * torch.tanh(0.0118 * (T - 1816.8)))

def dkappa_dT(T):
    sech2 = 1.0 - torch.tanh(0.0118 * (T - 1816.8))**2
    return 0.0184 + 225.10 * 0.0118 * sech2          # ← FIX: was 0.018
...
print(f"  DT_char = {DT_char:.1f} K  |  t_ref = {t_ref*1e3:.4f} ms")
print()
...
def bc_loss_fn(model, n=512):
    ...
    loss = loss + torch.mean((res_conv / bc_scale)**2)

    # Bottom z=Lz: Dirichlet T=T0
    x_bot = np.random.uniform(x_nd_min, x_nd_max, n)
    y_bot = np.random.uniform(y_nd_min, y_nd_max, n)
    z_bot = ten(np.full(n, z_nd_max))
    T_bot = model.T_phys(ten(x_bot), ten(y_bot), z_bot, ten(t_rnd))
    res_dir = (T_bot - T0) / DT_char
    loss = loss + torch.mean(res_dir**2)

    # Four insulated side faces: Neumann dT/dn = 0
    insulated_faces = [
    ...
    for fixed, val, r1, r2, length_scale in insulated_faces:
        ...
        dT_dn    = dT_dn_nd / length_scale 

        loss = loss + torch.mean((dT_dn * kap0 / bc_scale)**2)
...
def train(phase1_steps=12000, phase2_steps=8000,
    ...
    # Phase 2
    print("\n-- Phase 2: Adam fine-tuning (lr=1e-5) --")
    ...
    print(f"  Total wall-clock time : {total_time:.1f} s  ({total_time/60:.2f} min)")
...
# ─────────────────────────────────────────────────────────────────────────────
@torch.no_grad()
def eval_surface(model, t_phys, res=100):
    model.eval()
...
@torch.no_grad()
def eval_xz_plane(model, t_phys, res=100):
    model.eval()
    xs_p = np.linspace(0, Lx, res)
...
def get_melt_pool(model, t_phys, res=100):
    model.eval()
    xs_p = np.linspace(0, Lx, res)
    ys_p = np.linspace(0, Ly, res)
    zs_p = np.linspace(0, Lz, res)
