"""
Single-Track PINN for LPBF — Fair Comparison Version
=====================================================
Aligned with SPINN formulation for a controlled benchmark:

  CHANGES vs. original:
  [1] Hard IC ansatz  : T = T0 + t_nd * raw_net * DT_char
      → IC satisfied exactly by construction, no IC loss term
  [2] DT_char         : Rosenthal scale  q_peak * t_ref / (rho * cp0)
      → same non-dim temperature scale as SPINN
  [3] w_ic = 0.0      : IC loss term dropped (hard IC makes it redundant)
  [4] w_bc = 0.05     : matches SPINN BC weight
  [5] Peak LR = 5e-4  : same as SPINN (linear-scaling rule justified)
  [6] No L-BFGS       : replaced with Adam Phase-2 @ 1e-5 (matches SPINN)
  [7] dkdT analytical : avoids extra reverse-mode AD call
  [8] Wall-clock timer: printed for direct SPINN comparison
  [9] Param count     : printed for capacity comparison
  [10] Scaling table  : memory + time vs N, the core CoD evidence

Parameters from Safari & Wessels (2025) Table 1:
  rho     = 8351.91 kg/m³
...
def dkappa_dT(T):
    """
    Analytical derivative dk/dT [W/(m·K²)].
    [CHANGE 7] Avoids an extra reverse-mode grad1() call in the PDE loss,
    consistent with SPINN which also uses the analytical form.
    """
    sech2 = 1.0 - torch.tanh(0.018 * (T - 1816.8))**2
    return 0.0184 + 225.10 * 0.018 * sech2
...
# ─────────────────────────────────────────────────────────────────────────────
r_ref  = r_beam                          # lateral length scale [m]
t_ref  = r_beam**2 / alp0               # thermal diffusion time [s]

# [CHANGE 2] Rosenthal temperature scale — matches SPINN exactly.
# Original used DT_ref = qv_peak * r_beam**2 / kap0  (flux scale).
# SPINN uses  DT_char = qv_peak * t_ref / (rho * cp0) (energy/mass scale).
# Both are valid non-dimensionalisations; aligning them removes a confound.
DT_char = qv_peak * t_ref / (rho * cp0)

print(f"Non-dim scales (aligned with SPINN):")
print(f"  r_ref   = {r_ref*1e6:.0f} um")
print(f"  t_ref   = {t_ref*1e3:.4f} ms")
print(f"  DT_char = {DT_char:.1f} K  (Rosenthal)  "
      f"->  T_peak_est ~ {T0 + DT_char:.0f} K")
...
class PINN(nn.Module):
    """
    Monolithic MLP: 4 -> [width]^depth -> 1.

    [CHANGE 1] Hard IC ansatz replaces softplus output:
        T_phys = T0 + t_nd * net_raw * DT_char

    The factor t_nd guarantees T(x,y,z, t=0) = T0 for ALL network
    parameters, so no IC loss term is needed — exactly as in SPINN.
    The raw network output is unbounded (no softplus), which gives the
    optimiser a smoother landscape.
    """
    def __init__(self, width=128, depth=6):
        ...
        layers.append(nn.Linear(width, 1))   # unbounded raw output
    ...
    def raw(self, x_nd, y_nd, z_nd, t_nd):
        """Raw network output (unbounded scalar)."""
    ...
    def T_phys(self, x_nd, y_nd, z_nd, t_nd):
        """
        Physical temperature [K] with hard IC.
        T = T0 + t_nd * raw(x,y,z,t) * DT_char
        At t_nd=0: T = T0 exactly, for any network weights.
        """
        return T0 + t_nd * self.raw(x_nd, y_nd, z_nd, t_nd) * DT_char
...
def pde_loss_fn(model, x_nd, y_nd, z_nd, t_nd):
    """
    Residual of the nonlinear heat equation in physical units,
    normalised by qv_peak.

    [CHANGE 7] dkdT is now computed analytically (dkappa_dT) instead of
    via a second reverse-mode grad1() call. This is consistent with SPINN
    and reduces the computational graph depth by one level.
    """
    T_K = model.T_phys(x_nd, y_nd, z_nd, t_nd)

    # First-order derivatives of T_K w.r.t. non-dim coordinates
    dT_dt_nd = grad1(T_K, t_nd)
    dT_dx_nd = grad1(T_K, x_nd)
    dT_dy_nd = grad1(T_K, y_nd)
    dT_dz_nd = grad1(T_K, z_nd)

    # Second-order derivatives
    d2T_dx2_nd = grad1(dT_dx_nd, x_nd)
    d2T_dy2_nd = grad1(dT_dy_nd, y_nd)
    d2T_dz2_nd = grad1(dT_dz_nd, z_nd)

    # Chain rule: non-dim -> physical derivatives
    dT_dt   = dT_dt_nd   / t_ref
    dT_dx   = dT_dx_nd   / r_ref
    dT_dy   = dT_dy_nd   / r_ref
    dT_dz   = dT_dz_nd   / c_depth
    d2T_dx2 = d2T_dx2_nd / r_ref**2
    d2T_dy2 = d2T_dy2_nd / r_ref**2
    d2T_dz2 = d2T_dz2_nd / c_depth**2

    k    = kappa(T_K)
    c    = cp_fn(T_K)
    dkdT = dkappa_dT(T_K)   # [CHANGE 7] analytical, no extra AD call

    div_kgradT = (k * (d2T_dx2 + d2T_dy2 + d2T_dz2)
                  + dkdT * (dT_dx**2 + dT_dy**2 + dT_dz**2))

    # Physical coordinates for heat source
    x_p = x_nd * r_ref
    y_p = y_nd * r_ref
    z_p = z_nd * c_depth
    t_p = t_nd * t_ref
...
def bc_loss_fn(model, n=512):
    ...
    [CHANGE 3] IC loss term is gone — hard IC makes it redundant.
    [CHANGE 4] Normalisation uses DT_char (Rosenthal scale) to match SPINN.
    """
    t_rnd = np.random.uniform(t_nd_min, t_nd_max, n)
    loss  = torch.tensor(0.0, device=device)

    # ── Top surface z=0: Robin convection BC ─────────────────────────────
    # -kappa(T) * dT/dz|_{z=0} = h_conv * (T - T_inf)
    x_top = np.random.uniform(x_nd_min, x_nd_max, n)
    y_top = np.random.uniform(y_nd_min, y_nd_max, n)
    z_top = ten(np.zeros(n), grad=True)

    T_top    = model.T_phys(ten(x_top), ten(y_top), z_top, ten(t_rnd))
    dT_dz_nd = grad1(T_top, z_top)
    dT_dz    = dT_dz_nd / c_depth          # physical [K/m]
    k_top    = kappa(T_top)

    res_conv = -k_top * dT_dz - h_conv * (T_top - T_inf)
    # Normalise by h_conv * DT_char  [CHANGE 4]
    loss = loss + torch.mean((res_conv / (h_conv * DT_char))**2)

    # ── Five insulated faces: Neumann dT/dn = 0 ──────────────────────────
    insulated_faces = [
        ("x", x_nd_min, (y_nd_min, y_nd_max), (z_nd_min, z_nd_max), r_ref),
        ("x", x_nd_max, (y_nd_min, y_nd_max), (z_nd_min, z_nd_max), r_ref),
        ("y", y_nd_min, (x_nd_min, x_nd_max), (z_nd_min, z_nd_max), r_ref),
        ("y", y_nd_max, (x_nd_min, x_nd_max), (z_nd_min, z_nd_max), r_ref),
        ("z", z_nd_max, (x_nd_min, x_nd_max), (y_nd_min, y_nd_max), c_depth),
    ]

    for fixed, val, r1, r2, length_scale in insulated_faces:
        ...
        T_face   = model.T_phys(xb, yb, zb, ten(t_rnd))
        dT_dn_nd = grad1(T_face, wrt)
        dT_dn    = dT_dn_nd / length_scale   # physical [K/m]

        # Normalise by h_conv * DT_char  [CHANGE 4]
        loss = loss + torch.mean((dT_dn / (h_conv * DT_char))**2)
...
def sample_collocation(n_sobol=4096, n_laser=4096):
    """
    Sobol quasi-random base points + laser-focused enrichment.
    Identical strategy to original PINN — this is a fair comparison axis
    (PINN uses scattered points, SPINN uses per-axis factorised grid).
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# 10.  Training  — Two-phase Adam only (matches SPINN schedule)
# ─────────────────────────────────────────────────────────────────────────────
def train(phase1_steps=12000, phase2_steps=8000,
          w_pde=1.0, w_bc=0.05,          # [CHANGE 3,4] no w_ic, w_bc=0.05
          n_sobol=4096, n_laser=4096):
    """
    [CHANGE 5,6] Two-phase Adam schedule matching SPINN:
      Phase 1: lr=5e-4, CosineAnnealingWarmRestarts (T_0=2000, T_mult=2)
      Phase 2: lr=1e-5, fixed (fine-tuning)
    L-BFGS is removed — it is not used in SPINN, so including it would
    give PINN an unfair second-order optimisation advantage.

    [CHANGE 8] Wall-clock time is measured and reported.
    """
    model = PINN(width=128, depth=6).to(device)

    # [CHANGE 9] Print parameter count for capacity comparison
    n_params = count_params(model)
    print(f"  PINN parameter count : {n_params:,}")
    print(f"  (Compare with SPINN  : 4 × [1→64^4→32] axis-MLPs)\n")
    ...
    # [CHANGE 3] Only PDE + BC losses — no IC loss
    def losses():
    ...
    best_loss  = float("inf")
    best_state = None
    t_train_start = time.perf_counter()   # [CHANGE 8]

    # ── Phase 1: Adam with warm restarts ──────────────────────────────────
    # [CHANGE 5] Peak LR = 5e-4, same as SPINN
    print("-- Phase 1: Adam (lr=5e-4, cosine warm restarts) --")
    opt1   = torch.optim.Adam(model.parameters(), lr=5e-4)
    sched1 = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        opt1, T_0=2000, T_mult=2, eta_min=1e-5)
    ...
    for step in range(1, phase1_steps + 1):
        ...
        hist["wall_time"].append(time.perf_counter() - t_train_start)
        ...
        if step % 500 == 0 or step == 1:
            elapsed = hist["wall_time"][-1]
            print(f"  Step {step:6d} | lr={opt1.param_groups[0]['lr']:.2e} | "
                  f"Total:{lv:.3e} | PDE:{Lp.item():.3e} | "
                  f"BC:{Lb.item():.3e} | t={elapsed:.1f}s")

    model.load_state_dict(best_state)
    print(f"\n  Best Phase-1 loss: {best_loss:.3e}")

    # ── Phase 2: Adam fine-tuning at low LR ───────────────────────────────
    # [CHANGE 6] Matches SPINN Phase-2 (Adam @ 1e-5, fixed LR)
    print("\n-- Phase 2: Adam fine-tuning (lr=1e-5, fixed) --")
    ...
    # [CHANGE 8] Final timing report
    total_steps = phase1_steps + phase2_steps
    print(f"\n  ── Training Summary (PINN) ──────────────────────────────")
    print(f"  Total wall-clock time : {total_time:.1f} s  "
          f"({total_time/60:.2f} min)")
    print(f"  Total steps           : {total_steps:,}")
    print(f"  ms / step             : {1000*total_time/total_steps:.2f}")
    print(f"  Collocation pts       : {len(xi):,}  (scattered, not factorised)")
...
# ─────────────────────────────────────────────────────────────────────────────
# 11.  Scaling Table  — Core curse-of-dimensionality evidence
# ─────────────────────────────────────────────────────────────────────────────
def measure_pinn_scaling(model, n_list=(8, 16, 32, 64)):
    """
    [CHANGE 10] Measure PINN memory and time-per-step at different
    collocation densities N (total points = N^4 for a 4D problem).

    This is the key table that demonstrates the curse of dimensionality:
    PINN cost grows as O(N^4), SPINN grows as O(4N).

    For PINN, N^4 points are fed simultaneously into the MLP.
    Memory and time are measured for a single forward+backward pass.

    Usage:
        measure_pinn_scaling(model, n_list=[8, 16, 32, 64])
    """
    print("\n── PINN Scaling Table (curse-of-dimensionality evidence) ──")
    print(f"{'N':>6} | {'N^4 pts':>12} | {'mem (MB)':>10} | "
          f"{'ms/step':>10} | {'OOM':>5}")
    print("-" * 60)
    ...
    for N in n_list:
        ...
        try:
            ...
            L, Lp, Lb = (
                pde_loss_fn(model, pts[0], pts[1], pts[2], pts[3]),
                torch.tensor(0.0), torch.tensor(0.0)
            )
            Lp.backward() if False else L.backward()
            ...
            if device.type == "cuda":
                ...
                mem_mb = float("nan")   # CPU: use external profiler

            print(f"{N:>6} | {n_pts:>12,} | {mem_mb:>10.1f} | "
                  f"{elapsed_ms:>10.2f} | {'No':>5}")
            results.append((N, n_pts, mem_mb, elapsed_ms, False))
            ...
            if "out of memory" in str(e).lower():
                print(f"{N:>6} | {n_pts:>12,} | {'---':>10} | "
                      f"{'---':>10} | {'YES':>5}")
                results.append((N, n_pts, None, None, True))
                if device.type == "cuda":
                    torch.cuda.empty_cache()
            else:
                raise

    print("\n  Note: SPINN cost is O(4N) per axis, not O(N^4).")
    print("  At N=64: PINN needs 64^4 = 16M pts; SPINN needs 4x64 = 256 pts fed.")
    return results
...
def plot_snapshot(model, t_phys, fname="snapshot.png", res=150):
    ...
    def _draw_panel(ax, X, Y, T, title, xlabel, ylabel, ylim, invert_y=False):
        ...
        ax.contourf(X, Y, T_cl, levels=levels, vmin=VMIN, vmax=VMAX,
    ...
    _draw_panel(ax_xy, X_xy, Y_xy, T_xy,
                ...
                ylim=(0, Ly_mm), invert_y=False)
    _draw_panel(ax_xz, X_xz, Z_xz, T_xz,
                title=f"xz-plane (y={y_track*1e3:.2f} mm)  t={t_phys*1e3:.1f} ms",
                xlabel="x [mm]", ylabel="z [mm]",
                ylim=(Lz_mm, 0), invert_y=True)

    cbar_left_in = left_m_in + panel_w_in * 0.08
    cbar_w_in    = panel_w_in * 0.84
    cbar_h_in    = 0.30
    cbar_bot_in  = bot_m_in * 0.30
    cax = fig.add_axes(to_frac(cbar_left_in, cbar_bot_in, cbar_w_in, cbar_h_in))
    norm_cb = Normalize(vmin=VMIN - 100, vmax=VMAX + 100)
...
def plot_centreline(model, t_phys, fname="centreline.png"):
    ...
    ax.set_facecolor("white")
    ax.plot(xs_p * 1e3, T, color="#c0392b", lw=2.0, label="PINN (hard IC)")
...
def plot_losses(hist, fname="loss_history.png"):
    ...
    ax.axvline(phase1, color="orange", ls=":", lw=1.5,
               label="Phase 1 → Phase 2")
    ax.set_xlabel("Iteration", fontsize=12)
    ax.set_ylabel("Loss", fontsize=12)
    ax.set_title("Training Loss — PINN (hard IC, fair comparison)", fontsize=11)
    ...
    wt = hist["wall_time"]
    ax2.semilogy(wt, hist["total"], color="black",   lw=1.8, label="Total")
    ax2.semilogy(wt, hist["pde"],   color="#c0392b", lw=1.2, label="PDE")
    ax2.semilogy(wt, hist["bc"],    color="#27ae60", lw=1.2, ls=":", label="BC")
    ax2.set_xlabel("Wall-clock time [s]", fontsize=12)
    ax2.set_ylabel("Loss", fontsize=12)
    ax2.set_title("Loss vs Wall-Clock Time (compare directly with SPINN)", fontsize=11)
    ax2.legend(fontsize=10, framealpha=0.9)
    ax2.grid(True, which="both", alpha=0.25, color="gray", linestyle="--")
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    print("  PINN — Track 1  (Fair Comparison Version)")
    print("  Aligned with SPINN: hard IC, Rosenthal scale,")
    print("  two-phase Adam, w_bc=0.05, analytical dkdT")
    ...
    model, hist, total_time = train(
        phase1_steps = 12000,
        phase2_steps = 8000,
        w_pde  = 1.0,
        w_bc   = 0.05,    # [CHANGE 4] matches SPINN
        n_sobol = 524288,
        n_laser = 32768,
    )
    ...
    # [CHANGE 10] Scaling table — run BEFORE snapshots to show CoD
    print("\n-- Curse-of-Dimensionality Scaling Table --")
    measure_pinn_scaling(model, n_list=[8, 16, 32, 48, 64])
    ...
    for label, t_q in snapshots.items():
        ...
        plot_snapshot(model, t_q,
                      fname=f"results/snapshot_{label}_fair.png", res=150)
